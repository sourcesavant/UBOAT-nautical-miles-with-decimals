...
Shader "Amplify Impostors/Octahedron Impostor (Batched)"
{
	Properties
	{
		...
		_CullingFactor("Culling Factor (Size / Screen Relative Height)", Float) = 446.272645879
	...
	SubShader
	{
		...
		Pass
		{
			...
			v2f_surf vert_surf (ImpostorVertexData v ) {
				...
				float4 vertex = float4(v.vertex.xyz / 65535.0 * _PositionScale, 1.0);

				OctaImpostorVertex( vertex, normal, OCTAHEDRON_UV, o.uvsFrame1, o.uvsFrame2, o.uvsFrame3, o.octaFrame, o.viewPos, v.texcoord.z != 0.0 ? v.texcoord.z : 1.0, v.texcoord.w );
		...
		Pass
		{
			...
			UNITY_INSTANCING_BUFFER_START(Props)
                UNITY_DEFINE_INSTANCED_PROP(fixed, _IsFullDetailPresent)
			...
			v2f_surf vert_surf (ImpostorVertexData v ) {
				...
				fixed isFullDetailPresent = UNITY_ACCESS_INSTANCED_PROP(Props, _IsFullDetailPresent);

				if(isFullDetailPresent != 0 && length(centralWorldPos - _WorldSpaceCameraPos) < _CullingFactor * 0.5 * _LODBias * v.texcoord.z * unity_CameraProjection._m11)
					o.pos.xyzw = -10;
			...
			, out float outDepth : SV_Depth
				...
				float4 clipPos;
				float3 worldPos;
				OctaImpostorFragment( o, clipPos, worldPos, IN.sh.w, IN.uvsFrame1, IN.uvsFrame2, IN.uvsFrame3, IN.octaFrame, IN.viewPos, IN.sh.w );
		...
		Pass
		{
			...
			fixed4 frag_surf (v2f_surf IN, out float outDepth : SV_Depth ) : SV_Target {
				...
				float4 clipPos;
				float3 worldPos;
				OctaImpostorFragment( o, clipPos, worldPos, 0, IN.uvsFrame1, IN.uvsFrame2, IN.uvsFrame3, IN.octaFrame, IN.viewPos, IN.angle );
				IN.pos.zw = clipPos.zw;
