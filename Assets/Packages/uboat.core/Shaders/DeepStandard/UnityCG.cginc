...
half3 SHEvalLinearL0L1 (half4 normal)
{
    ...
    x.r = dot(ambient_SHAr,normal);
    x.g = dot(ambient_SHAg,normal);
    x.b = dot(ambient_SHAb,normal);
...
half3 SHEvalLinearL2 (half4 normal)
{
    ...
    x1.r = dot(ambient_SHBr,vB);
    x1.g = dot(ambient_SHBg,vB);
    x1.b = dot(ambient_SHBb,vB);

    // Final (5th) quadratic (L2) polynomial
    half vC = normal.x*normal.x - normal.y*normal.y;
    x2 = ambient_SHC.rgb * vC;
...
#define UNITY_CALC_FOG_FACTOR(coord) UNITY_CALC_FOG_FACTOR_RAW(UNITY_Z_0_FAR_FROM_CLIPSPACE(coord) * 2.0)
