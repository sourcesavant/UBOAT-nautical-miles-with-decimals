...
#pragma warning restore 414
    ...
    abstract public class ShadowmapBase
    {
        ...
        protected          CullingResults           m_CullResults; // TODO: Temporary, due to CullResults dependency in ShadowUtils' matrix extraction code. Remove this member once that dependency is gone.
        ...
        public void Assign(CullingResults cullResults) { m_CullResults = cullResults; }            // TODO: Remove when m_CullResults is removed again
        abstract public bool Reserve(FrameId frameId, Camera camera, bool cameraRelativeRendering, ref ShadowData shadowData, ShadowRequest sr, uint width, uint height, ref VectorArray<ShadowData> entries, ref VectorArray<ShadowPayload> payloads, NativeArray<VisibleLight> lights, ShadowmapRenderFlags staticShadowMap);
        abstract public bool Reserve(FrameId frameId, Camera camera, bool cameraRelativeRendering, ref ShadowData shadowData, ShadowRequest sr, uint[] widths, uint[] heights, ref VectorArray<ShadowData> entries, ref VectorArray<ShadowPayload> payloads, NativeArray<VisibleLight> lights, ShadowmapRenderFlags staticShadowMap);
        ...
        abstract public void Update(FrameId frameId, ScriptableRenderContext renderContext, CommandBuffer cmd, CullingResults cullResults, NativeArray<VisibleLight> lights);
    ...
    public interface IShadowManager
    {
        ...
        void ProcessShadowRequests(FrameId frameId, CullingResults cullResults, Camera camera, bool cameraRelativeRendering, NativeArray<VisibleLight> lights, ref uint shadowRequestsCount, int[] shadowRequests, out int[] shadowDataIndices);
        // Renders all shadows for lights the were deemed shadow casters after the last call to ProcessShadowRequests
        void RenderShadows(FrameId frameId, ScriptableRenderContext renderContext, CommandBuffer cmd, CullingResults cullResults, NativeArray<VisibleLight> lights);
    ...
    {
        public  abstract void ProcessShadowRequests(FrameId frameId, CullingResults cullResults, Camera camera, bool cameraRelativeRendering, NativeArray<VisibleLight> lights, ref uint shadowRequestsCount, int[] shadowRequests, out int[] shadowDataIndices);
        public  abstract void RenderShadows(FrameId frameId, ScriptableRenderContext renderContext, CommandBuffer cmd, CullingResults cullResults, NativeArray<VisibleLight> lights);
        ...
        protected abstract void PrioritizeShadowCasters(Camera camera, NativeArray<VisibleLight> lights, uint shadowRequestsCount, int[] shadowRequests);
        // prune the shadow requests - may modify shadowRequests and shadowsCountshadowRequestsCount
        protected abstract void PruneShadowCasters(Camera camera, NativeArray<VisibleLight> lights, ref VectorArray<int> shadowRequests, ref VectorArray<ShadowmapBase.ShadowRequest> requestsGranted, out uint totalRequestCount);
        ...
        protected abstract bool AllocateShadows(FrameId frameId, Camera camera, bool cameraRelativeRendering, NativeArray<VisibleLight> lights, uint totalGranted, ref VectorArray<ShadowmapBase.ShadowRequest> grantedRequests, ref VectorArray<int> shadowIndices, ref VectorArray<ShadowData> shadowmapDatas, ref VectorArray<ShadowPayload> shadowmapPayload);
