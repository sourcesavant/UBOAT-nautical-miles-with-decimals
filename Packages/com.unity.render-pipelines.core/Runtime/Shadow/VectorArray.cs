...
namespace UnityEngine.Experimental
{
    public struct VectorArray<T>
    {
        ...
        public T[]  m_array;           // backing storage
	    public uint m_offset;          // offset into backing storage
	    public uint m_count;           // active slots, <= m_array.Length
        ...
        public void Sort()
        {
	        ...
	        Profiler.BeginSample("VectorArray.SlowSort");
			Array.Sort(m_array, (int)m_offset, (int)m_count);
			//Quick_Sort(m_array, (int)m_offset, (int)(m_offset + m_count - 1));
	        Profiler.EndSample();
		}
        ...
        public void Sort(Comparison<T> comparer)
        {
	        ...
	        Profiler.BeginSample("VectorArray.Sort");
	        QuickSort(m_array, (int)m_offset, (int)(m_offset + m_count - 1), comparer);
	        Profiler.EndSample();
        }

	    private static void QuickSort(T[] arr, int left, int right, Comparison<T> comparison)
	    {
		    while (true)
		    {
			    if (left < right)
			    {
				    int pivot;
				    T pivotItem = arr[left];

				    while(true)
				    {
					    while(comparison(arr[left], pivotItem) < 0)
						    left++;

					    while(comparison(arr[right], pivotItem) > 0)
						    right--;
	    ...
	    public static void QuickSort<U>(U[] arr, int left, int right) where U : IComparable<U>
	    {
		    while(true)
		    {
			    if(left < right)
			    {
				    ...
				    while(true)
				    {
					    ...
					    if(left1 < right1)
					    {
						    if(arr[left1].CompareTo(arr[right1]) == 0)
						    {
							    pivot = right1;
							    break;
						    }
