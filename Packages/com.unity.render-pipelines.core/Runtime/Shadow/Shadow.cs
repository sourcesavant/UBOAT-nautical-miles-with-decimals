...
namespace UnityEngine.Rendering
{
    ...
    public class ShadowAtlas : ShadowmapBase, IDisposable
    {
		...
		protected readonly int						  g_vLightDirWs;
	    ...
	    private            Material                   m_DepthBlitMat;

	    private static readonly int _SourceDepthTex = Shader.PropertyToID("_SourceDepthTex");
		private static readonly int _CopyRect = Shader.PropertyToID("_CopyRect");
	    ...
	    public struct Data
        {
			...
			public ShadowmapRenderFlags shadowmapRenderFlags;
            public uint                 slice;
        ...
        public class CachedEntry : IComparable<CachedEntry>
        {
            public Key  key;
            public Data current;
	        public Rect previousRect;
	        public ShadowmapRenderFlags previousShadowmapRenderFlags;
			public bool zclip;
		...
		private static readonly Rect[] faceViewports =
		{
			new Rect(0.0f, 0.0f, 0.333333f, 0.5f),
			new Rect(0.333333f, 0.0f, 0.333333f, 0.5f),
			new Rect(0.666666f, 0.0f, 0.333333f, 0.5f),
			new Rect(0.0f, 0.5f, 0.333333f, 0.5f),
			new Rect(0.333333f, 0.5f, 0.333333f, 0.5f),
			new Rect(0.666666f, 0.5f, 0.333333f, 0.5f)
		};

	    private static readonly Rect[] faceViewports512 =
	    {
		...
		public ShadowAtlas(ref AtlasInit init) : base(ref init.baseInit)
        {
	        ...
	        m_DepthBlitMat = CoreUtils.CreateEngineMaterial(Shader.Find("Hidden/SRP-BlitCopyDepth"));
	        ...
	        g_vLightDirWs = Shader.PropertyToID("g_vLightDirWs");
        ...
        override public bool Reserve(FrameId frameId, Camera camera, bool cameraRelativeRendering, ref ShadowData shadowData, ShadowRequest sr, uint width, uint height, ref VectorArray<ShadowData> entries, ref VectorArray<ShadowPayload> payload, NativeArray<VisibleLight> lights, ShadowmapRenderFlags shadowmapRenderFlags)
        {
            ...
            return Reserve(frameId, camera, cameraRelativeRendering, ref shadowData, sr, m_TmpWidths, m_TmpHeights, ref entries, ref payload, lights, shadowmapRenderFlags);
        }

        override public bool Reserve(FrameId frameId, Camera camera, bool cameraRelativeRendering, ref ShadowData shadowData, ShadowRequest sr, uint[] widths, uint[] heights, ref VectorArray<ShadowData> entries, ref VectorArray<ShadowPayload> payload, NativeArray<VisibleLight> lights, ShadowmapRenderFlags shadowmapRenderFlags)
        {
           ...
           // uint originalActiveEntries = m_ActiveEntriesCount;
	        ...
	        var light = lights[sr.index].light;
            AdditionalShadowData asd = light.GetComponent<AdditionalShadowData>();
            if (!asd)
                return false;
            ...
            while (facecnt > 0)
            {
                if ((bit & facemask) != 0)
                {
	                ...
	                var ce = asd.cachedData[key.faceIdx].shadowDataEntry;
					
					if (ce == null)
						asd.cachedData[key.faceIdx].shadowDataEntry = ce = new CachedEntry();

	                if (m_ActiveEntriesCount < m_EntryCache.Count())
		                m_EntryCache[m_ActiveEntriesCount++] = ce;
	                else
		                m_EntryCache.Add(ce);

	                if (ce.current.viewport.width == width && ce.current.viewport.height == height && ce.current.frameId.frameCount + 1 == frameId.frameCount)
	                {
		                ce.previousRect = ce.current.viewport;
		                ce.previousShadowmapRenderFlags = ce.current.shadowmapRenderFlags;
	                }
	                else
	                {
		                ce.previousRect = default(Rect);
		                ce.previousShadowmapRenderFlags = 0;
                    ...
                    {
						if((!asd.isTransformStatic && (ce.current.shadowmapRenderFlags & ShadowmapRenderFlags.DontRefresh) == 0) || ce.previousRect.width == 0.0f)
						{
							...
							/*vp = */ShadowUtils.ExtractPointLightMatrix(lights[sr.index], key.faceIdx, guardAngle, out ce.current.view, out ce.current.proj, out asd.cachedData[key.faceIdx].devproj, out asd.cachedData[key.faceIdx].invvp, out ce.current.lightDir, out ce.current.splitData);
							asd.cachedData[key.faceIdx].invvpTranspose = asd.cachedData[key.faceIdx].invvp.transpose;
						}
                    }
                    else if (sr.shadowType == GPUShadowType.Spot)
                    {
                        if((!asd.isTransformStatic && (ce.current.shadowmapRenderFlags & ShadowmapRenderFlags.DontRefresh) == 0) || ce.previousRect.width == 0.0f)
						{
							...
							/*vp = */ShadowUtils.ExtractSpotLightMatrix(lights[sr.index], guardAngle, out ce.current.view, out ce.current.proj, out asd.cachedData[key.faceIdx].devproj, out asd.cachedData[key.faceIdx].invvp, out ce.current.lightDir, out ce.current.splitData);
							asd.cachedData[key.faceIdx].invvpTranspose = asd.cachedData[key.faceIdx].invvp.transpose;
						}
					}
                    else if (sr.shadowType == GPUShadowType.Directional)
                    {
                        /*vp = */ShadowUtils.ExtractDirectionalLightMatrix(lights[sr.index], key.faceIdx, cascadeCnt, cascadeRatios, nearPlaneOffset, width, height, out ce.current.view, out ce.current.proj, out asd.cachedData[key.faceIdx].devproj, out asd.cachedData[key.faceIdx].invvp, out ce.current.lightDir, out ce.current.splitData, m_CullResults, (int)sr.index);
                        ...
                        /*vp = */asd.cachedData[key.faceIdx].invvp = asd.cachedData[key.faceIdx].devproj = Matrix4x4.identity; // should never happen, though
                    ...
                    if (cameraRelativeRendering)
                    {
                        ...
                        translation[15] = 1.0f;
	                    asd.cachedData[key.faceIdx].invvp = translation * asd.cachedData[key.faceIdx].invvp;
                        if (sr.shadowType == GPUShadowType.Directional)
                        {
	                ...
	                ce.current.shadowmapRenderFlags = shadowmapRenderFlags;
					//m_EntryCache[ceIdx] = ce;

                    if (sr.shadowType == GPUShadowType.Directional)
                        sd.pos = new Vector3(ce.current.view.m03, ce.current.view.m13, ce.current.view.m23);
                    else
                        sd.pos = cameraRelativeRendering ? (light.transform.position - camera.transform.position) : light.transform.position;

                    sd.shadowToWorld = asd.cachedData[key.faceIdx].invvpTranspose;
                    sd.proj          = new Vector4(asd.cachedData[key.faceIdx].devproj.m00, asd.cachedData[key.faceIdx].devproj.m11, asd.cachedData[key.faceIdx].devproj.m22, asd.cachedData[key.faceIdx].devproj.m23);
        ...
        override public void Update(FrameId frameId, ScriptableRenderContext renderContext, CommandBuffer cmd, CullingResults cullResults, NativeArray<VisibleLight> lights)
        {
            ...
            for (uint i = 0; i < m_ActiveEntriesCount; ++i)
            {
	            var entryCache = m_EntryCache[i];
	            bool renderDynamic = true;

				if (!cullResults.GetShadowCasterBounds(entryCache.key.visibleIdx, out bounds))
	            {
					if((entryCache.current.shadowmapRenderFlags & ShadowmapRenderFlags.CacheStaticRenderers) == 0)
						continue;
					else
						renderDynamic = false;
				}

				var light = lights[entryCache.key.visibleIdx].light;
				var asd = light.GetComponent<AdditionalShadowData>();

	            if((entryCache.current.shadowmapRenderFlags & ShadowmapRenderFlags.DontRefresh) != 0 && entryCache.current.viewport == entryCache.previousRect && entryCache.current.shadowmapRenderFlags == entryCache.previousShadowmapRenderFlags)
		            continue;

	            entryCache.previousRect = entryCache.current.viewport;

				uint entrySlice = entryCache.current.slice;
                ...
                cmd.SetViewport(entryCache.current.viewport);
                cmd.SetViewProjectionMatrices(entryCache.current.view, entryCache.current.proj);
                cmd.SetGlobalVector(g_vLightDirWs, entryCache.current.lightDir);
                cmd.SetGlobalFloat(m_ZClipId, entryCache.zclip ? 1.0f : 0.0f);
                //if (captureFrame)
                //    cmd.EndSample(cbName);

                dss.lightIndex = entryCache.key.visibleIdx;
                dss.splitData = entryCache.current.splitData;

				if(renderDynamic)
					renderDynamic = (entryCache.current.shadowmapRenderFlags & ShadowmapRenderFlags.RenderDynamicRenderers) != 0;
				
				if ((entryCache.current.shadowmapRenderFlags & ShadowmapRenderFlags.CacheStaticRenderers) != 0)
	            {
		            var cachedTex = asd.cachedShadowmap;

		            if (cachedTex)
		            {
			            if (cachedTex.width < entryCache.current.viewport.width * (light.type == LightType.Point ? 3 : 1))
			            {
				            Object.DestroyImmediate(cachedTex);
				            asd.cachedShadowmap = cachedTex = null;
				            asd.cachedShadowmapFaces = 0;
			            }
						else if (!cachedTex.IsCreated())
			            {
				            asd.cachedShadowmapFaces = 0;
				            cachedTex.Create();
			            }
		            ...
		            int faceIdx = (int) entryCache.key.faceIdx;
		            var rect = light.type == LightType.Point ? faceViewports[faceIdx] : new Rect(0.0f, 0.0f, 1.0f, 1.0f);

					if ((asd.cachedShadowmapFaces & (1 << faceIdx)) == 0)
					{
						asd.cachedShadowmapFaces |= 1 << faceIdx;
						...
						if (!cachedTex)
						{
							switch (light.type)
				            {
					            case LightType.Point:
						            asd.cachedShadowmap = cachedTex = CreateCachedShadowmap((int) entryCache.current.viewport.width * 3, (int) entryCache.current.viewport.height * 2, light.name + " Cached Shadowmap");
						            asd.cachedShadowmapWidth = (int) entryCache.current.viewport.width * 3;
						            asd.cachedShadowmapHeight = (int) entryCache.current.viewport.height * 2;

						            break;

					            case LightType.Spot:
						            asd.cachedShadowmap = cachedTex = new RenderTexture((int) entryCache.current.viewport.width, (int) entryCache.current.viewport.height, 32,
							            RenderTextureFormat.Depth, RenderTextureReadWrite.Default);
									asd.cachedShadowmapWidth = (int)entryCache.current.viewport.width;
									asd.cachedShadowmapHeight = (int)entryCache.current.viewport.height;
						            break;

					            default:
						            throw new InvalidOperationException("Unsupported light type for cached shadowmaps.");
				            }
						}

						cmd.SetRenderTarget(cachedTex);
			            
			            var scaledRect = rect;
			            scaledRect.xMin *= asd.cachedShadowmapWidth;
			            scaledRect.yMin *= asd.cachedShadowmapHeight;
			            scaledRect.xMax *= asd.cachedShadowmapWidth;
			            scaledRect.yMax *= asd.cachedShadowmapHeight;
			            cmd.SetViewport(scaledRect);
			            ...
			            if (!tempCamera)
			            {
				            var tempCameraGo = new GameObject("SRPTempCamera");
				            tempCameraGo.hideFlags = HideFlags.HideAndDontSave;
				            tempCameraGo.SetActive(false);
				            tempCamera = tempCameraGo.AddComponent<Camera>();
				            tempCamera.fieldOfView = 90.0f;
				            tempCamera.nearClipPlane = 0.065f;
			            }

			            tempCamera.transform.position = light.transform.position;
			            tempCamera.transform.rotation = entryCache.current.view.rotation;
			            tempCamera.farClipPlane = light.range * 1.25f;
			            tempCamera.projectionMatrix = entryCache.current.proj;

						// cull start

						if (tempCamera.TryGetCullingParameters(out var cullingParams))
			            {
				            cullingParams.cullingMask = ~(uint)light.cullingMask;
				            cullingParams.shadowDistance = 0.0f;
				            cullingParams.reflectionProbeSortingCriteria = ReflectionProbeSortingCriteria.None;
				            cullingParams.cullingOptions = CullingOptions.DisablePerObjectCulling | CullingOptions.ForceEvenIfCameraIsNotActive;

				            var cullResults2 = renderContext.Cull(ref cullingParams);
				            // cull stop
				            ...
				            var drawSettings = new DrawingSettings
				            {
                                ...
                                {
						            cameraPosition = -entryCache.current.view.GetColumn(3),
                                    distanceMetric = DistanceMetric.Perspective,
						            worldToCameraMatrix = entryCache.current.view,
						            criteria = SortingCriteria.CommonOpaque
					            },
                                perObjectData = PerObjectData.None
				            };
				            drawSettings.SetShaderPassName(0, new ShaderTagId("ShadowCaster"));

				            var filterSettings = new FilteringSettings
				            {
					            layerMask = ~light.cullingMask,
					            renderingLayerMask = uint.MaxValue,
					            renderQueueRange = new RenderQueueRange {lowerBound = 0, upperBound = 3001}
				            };
				            renderContext.DrawRenderers(cullResults2, ref drawSettings, ref filterSettings);
			            }
						
						// restore render target
						cmd.SetRenderTarget(m_ShadowmapId, 0, (CubemapFace) 0, (int) curSlice);
			            cmd.SetViewport(entryCache.current.viewport);
			            renderContext.ExecuteCommandBuffer(cmd);
			            cmd.Clear();
		            }

					cmd.SetGlobalTexture(_SourceDepthTex, cachedTex);
		            cmd.SetGlobalVector(_CopyRect, new Vector4(rect.width * asd.cachedShadowmapWidth / cachedTex.width, rect.height * asd.cachedShadowmapHeight / cachedTex.height, rect.xMin * asd.cachedShadowmapWidth / cachedTex.width, rect.yMin * asd.cachedShadowmapHeight / cachedTex.height));
		            CoreUtils.DrawFullScreen(cmd, m_DepthBlitMat, null, 0);
	    ...
	    private static RenderTexture CreateCachedShadowmap(int width, int height, string name)
	    {
			var cachedTex = new RenderTexture(width, height, 32, RenderTextureFormat.Depth, RenderTextureReadWrite.Default);

		    cachedTex.hideFlags = HideFlags.DontSave;
		    cachedTex.name = name;
		    cachedTex.useMipMap = false;
		    cachedTex.autoGenerateMips = false;
		    cachedTex.filterMode = FilterMode.Bilinear;
		    cachedTex.useDynamicScale = false;
		    cachedTex.enableRandomWrite = false;
		    cachedTex.vrUsage = VRTextureUsage.None;
		    cachedTex.wrapMode = TextureWrapMode.Clamp;
		    cachedTex.memorylessMode = RenderTextureMemoryless.None;
        ...
        protected bool Alloc(FrameId frameId, Key key, uint width, uint height, out uint cachedEntryIdx, VectorArray<ShadowPayload> payload)
        {
            ...
            if (m_EntryCache.FindFirst(out idx, ref key, m_Comparator))
            {
                ...
                {
                    //ce.previousRect = m_EntryCache[idx].current.viewport;
        ...
        protected bool Layout()
        {
            ...
            //tmp.Sort();
	        Profiler.BeginSample("VectorArray<CachedEntry>.Sort");
	        QuickSort(tmp.m_array, (int)tmp.m_offset, (int)(tmp.m_offset + tmp.m_count - 1));
	        Profiler.EndSample();
	    ...
	    private static void QuickSort(CachedEntry[] arr, int left, int right)
	    {
		    while(true)
		    {
			    if(left < right)
			    {
				    int pivot;
				    var pivotItem = arr[left];
				    int left1 = left;
				    int right1 = right;

				    while(true)
				    {
					    while(arr[left1].CompareTo(pivotItem) < 0)
						    left1++;

					    while(arr[right1].CompareTo(pivotItem) > 0)
						    right1--;
...
// -------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    public class ShadowVariance : ShadowAtlas
    {
        ...
        protected override void PostUpdate(FrameId frameId, CommandBuffer cb, uint rendertargetSlice, NativeArray<VisibleLight> lights)
        {
            ...
            uint cnt = (uint)Mathf.Min(m_ActiveEntriesCount, m_EntryCache.Count());
            uint i = 0;
...
// -------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    public class ShadowManager : ShadowManagerBase
    {
        ...
        private static readonly Comparison<long> reverseCompareComparer = RerverseCompare;
        ...
        protected override void PrioritizeShadowCasters(Camera camera, NativeArray<VisibleLight> lights, uint shadowRequestsCount, int[] shadowRequests)
        {
            ...
            m_TmpSortKeys.Sort(reverseCompareComparer);
            m_TmpSortKeys.ExtractTo(shadowRequests, 0, out shadowRequestsCount, m_Extractor);
        }

	    private static int RerverseCompare(long lhs, long rhs)
	    {
		    return rhs.CompareTo(lhs);
	    }

		protected override void PruneShadowCasters(Camera camera, NativeArray<VisibleLight> lights, ref VectorArray<int> shadowRequests, ref ShadowRequestVector requestsGranted, out uint totalRequestCount)
        {
            ...
            for (uint i = 0, count = shadowRequests.Count(); i < count && totalSlots > 0; ++i)
            {
	            ...
	            var light = vl.light;
                GPUShadowType shadowType = GetShadowLightType(light);
                ...
                if (add)
                {
                    sreq.instanceId = light.GetInstanceID();
        ...
        protected override bool AllocateShadows(FrameId frameId, Camera camera, bool cameraRelativeRendering, NativeArray<VisibleLight> lights, uint totalGranted, ref ShadowRequestVector grantedRequests, ref ShadowIndicesVector shadowIndices, ref ShadowDataVector shadowDatas, ref ShadowPayloadVector shadowmapPayload)
        {
            ...
            for (uint i = 0, cnt = grantedRequests.Count(); i < cnt; ++i)
            {
                ...
                {
                    if (m_ShadowmapsPerType[(int)shadowtype, smidx] != null && m_ShadowmapsPerType[(int)shadowtype, smidx].Reserve(frameId, camera, cameraRelativeRendering, ref sd, grantedRequests[i], (uint)asd.shadowResolution, (uint)asd.shadowResolution, ref shadowDatas, ref shadowmapPayload, lights, asd.shadowmapRenderFlags))
                        break;
                    smidx++;
                }
